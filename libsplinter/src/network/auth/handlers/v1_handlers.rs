...
use crate::network::auth::{
    AuthorizationAction, AuthorizationManagerStateMachine, AuthorizationMessage, AuthorizationState,
};
...
impl Handler for AuthTrustRequestHandler {
    ...
    fn handle(
        ...
        match self.auth_manager.next_remote_state(
            ...
            Ok(AuthorizationState::TrustIdentified(identity)) => {
                debug!(
...
impl Handler for AuthTrustResponseHandler {
    ...
    fn handle(
        ...
        match self.auth_manager.next_state(
            context.source_connection_id(),
            AuthorizationAction::TrustIdentifying(self.identity.clone()),
        ) {
            Err(err) => {
                warn!(
            ...
            Ok(AuthorizationState::TrustIdentified(_)) => (),
            Ok(next_state) => panic!("Should not have been able to transition to {}", next_state),
        }
...
impl Handler for AuthCompleteHandler {
    ...
    fn handle(
        ...
        match self.auth_manager.next_state(
            ...
            Ok(AuthorizationState::Authorized(_)) | Ok(AuthorizationState::AuthComplete(_)) => (),
            Ok(next_state) => panic!("Should not have been able to transition to {}", next_state),
        }
