...
impl fmt::Display for AuthorizationState {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str(match self {
            ...
            #[cfg(feature = "trust-authorization")]
            AuthorizationState::ProtocolAgreeing => "ProtocolAgreeing",
            AuthorizationState::TrustV0(action) => &format!("TrustV0: {}", action),
            #[cfg(feature = "trust-authorization")]
            AuthorizationState::Trust(action) => &format!("Trust: {}", action),
        ...
        match &cur_state.local_state {
            ...
            AuthorizationState::TrustV0(state) => match action {
                AuthorizationAction::TrustV0(action) => {
                    ...
                    cur_state.local_state = new_state;
                    Ok(new_state)
                }
                _ => Err(AuthorizationActionError::InvalidMessageOrder(
                    AuthorizationState::TrustV0(state.clone()),
                    action,
                )),
            },
            ...
            AuthorizationState::Trust(state) => match action {
                ...
                _ => Err(AuthorizationActionError::InvalidMessageOrder(
                    AuthorizationState::Trust(state.clone()),
                    action,
                )),
            },
            _ => Err(AuthorizationActionError::InvalidMessageOrder(
