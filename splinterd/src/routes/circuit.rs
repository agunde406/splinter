...
use splinter::circuit::{
    AuthorizationType, CircuitBuildError, DurabilityType, PersistenceType, Roster, RouteType,
    SplinterState,
};
...
impl StdError for CircuitRouteError {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            CircuitRouteError::BuildError(err) => Some(err),
...
impl std::fmt::Display for CircuitRouteError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            CircuitRouteError::BuildError(err) => write!(f, "Unable to build circuit: {}", err),
            CircuitRouteError::NotFound(msg) => write!(f, "Circuit not found: {}", msg),
        }
    }
}

impl From<CircuitBuildError> for CircuitRouteError {
    fn from(err: CircuitBuildError) -> Self {
        CircuitRouteError::BuildError(err)
    }
}
...
pub fn fetch_circuit(
    ...
    Box::new(
        web::block(move || {
            let circuit = {
                let state = state.read().expect("Splinter state lock was poisoned");
...
fn query_list_circuits(
    ...
    web::block(move || {
        let circuits = state
            .read()
            .expect("Splinter state lock was poisoned")
            .circuits()
            .clone();
